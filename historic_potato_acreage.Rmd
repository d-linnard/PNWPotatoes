...
JoinedCounties <- inner_join(wa_county, MuAcreageDf, by = "subregion")
...
#MIGHT NOT NEED TO DO. Spreading data, making long data wide so we can make an acreage map for each year
#SpreadAcreage <- spread(ShortHistoricAcreage, Year,Value)

#Converting NA to 0 (For counties included in the data set for years when they did not report any acreage, we will consider that 0 acreages.) In this case, 0 is more of a place holder since we don't necessarily know that 0 acreages were reported for the county, we just know that nothing was reported for the county in years we designate with the 0 place holder.
...
#Gather plots
GatheredAcreage <- JoinedCounties %>%
gather(subregion, Value, -Year) %>% #gather data from all columns except year (will be it's own column in new df) and places it into new columns: subregion and value
    mutate(series = factor(subregion,
                           levels = c('Acreage1965', 'Acreage1966', 'Acreage1967', 'Acreage1968', 'Acreage1969', 'Acreage1970', 'Acreage1971', 'Acreage1972', 'Acreage1973', 'Acreage1974', 'Acreage1975', 'Acreage1976', 'Acreage1977', 'Acreage1978', 'Acreage1979', 'Acreage1980', 'Acreage1981', 'Acreage1982', 'Acreage1983', 'Acreage1984', 'Acreage1985', 'Acreage1986', 'Acreage1987', 'Acreage1988', 'Acreage1989', 'Acreage1990', 'Acreage1991', 'Acreage1992', 'Acreage1993', 'Acreage1994', 'Acreage1995', 'Acreage1996', 'Acreage1997', 'Acreage1998', 'Acreage1999', 'Acreage2000', 'Acreage2001', 'Acreage2002', 'Acreage2003', 'Acreage2004', 'Acreage2005', 'Acreage2006', 'Acreage2007', 'Acreage2008', 'Acreage2009', 'Acreage2010', 'Acreage2011', 'Acreage2012', 'Acreage2014')))
...
Acreage1965 <- ggplot() + 
  ...
  ggtitle("WA County Potato Acreage 1965")
#loop attempt 
county.graphs <-function(JoinedSpreadCounties, na.rm = TRUE, ...){
  county_list <- unique(JoinedSpreadCounties$subregion)
  for (i in seq_along(county_list)) {
    plot <- ggplot(subset(JoinedSpreadCounties, JoinedSpreadCounties==county.list[i]),
                   aes(x=long, y=lat, group=group,
                color="black", lwd=0.3, alpha=0.8))
    print(plot)
  }
...
# Gather map plots
JoinedSpreadCounties %>%
  gather(key="subregion", value="Acreage1965")%>%
  ggplot(us.states,
       aes(x=long, y=lat, group=group))+
  geom_path(size=0.25)+
  geom_path(data=ca.provinces, size=0.25,color="grey60")+
...
for (c in seq_along(acreage_cols)){
 ...
 coord_map("bonne", lat0 = 50,
  ...
  xlab("Longitude") +
  ylab("Latitude") +
  facet_wrap(~ acreage_cols, ncol= 4, scales="free")
}
```
